#!/usr/bin/env python3
"""Pluggable credential storage for smokeball-mcp.

Secrets (OAuth client ID, client secret, API key, region) are stored in the
...
Storage order (read):  OS keyring  ->  process env  ->  ~/.smokeball-mcp/.env file
...
user opts out of keyring with ``SMOKEBALL_MCP_USE_KEYRING=0``.
...
_USE_KEYRING_FLAG = "SMOKEBALL_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``SMOKEBALL_MCP_USE_KEYRING=0`` (or false/no/off). A failing or
