...
@mcp.tool()
def update_firm(name: str = "", phone: str = "", email: str = "",
                address: str = "") -> str:
...
@mcp.tool()
def update_firm_user_mapping(mapping_id: str, staff_id: str = "",
                              email: str = "") -> str:
    ...
    if email:
        fields["email"] = email
    return json.dumps(SmokeBallClient().update_firm_user_mapping(mapping_id, **fields), indent=2)
...
def search_staff(query: str = "", limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().search_staff(
        query=query or None, limit=limit, offset=offset), indent=2)
...
@mcp.tool()
def create_staff_member(first_name: str, last_name: str, email: str,
                        role: str = "") -> str:
...
@mcp.tool()
def update_staff_member(staff_id: str, first_name: str = "", last_name: str = "",
    ...
    if role:
        fields["role"] = role
    return json.dumps(SmokeBallClient().update_staff_member(staff_id, **fields), indent=2)
...
def list_contacts(limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().list_contacts(limit=limit, offset=offset), indent=2)
...
@mcp.tool()
def update_contact(contact_id: str, first_name: str = "", last_name: str = "",
                   email: str = "", phone: str = "") -> str:
...
def get_contact_relation(contact_id: str, relation_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_contact_relation(contact_id, relation_id), indent=2)


@mcp.tool()
def create_contact_relation(contact_id: str, related_contact_id: str,
    ...
    return json.dumps(SmokeBallClient().create_contact_relation(contact_id, **fields), indent=2)


@mcp.tool()
def update_contact_relation(contact_id: str, relation_id: str,
                             relation_type: str) -> str:
    """Update a contact relationship type."""
    return json.dumps(SmokeBallClient().update_contact_relation(
        contact_id, relation_id, relationType=relation_type), indent=2)


@mcp.tool()
def delete_contact_relation(contact_id: str, relation_id: str) -> str:
    """Delete a relationship from a contact."""
    return json.dumps(SmokeBallClient().delete_contact_relation(contact_id, relation_id), indent=2)
...
def list_matters(limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().list_matters(limit=limit, offset=offset), indent=2)
...
@mcp.tool()
def update_matter(matter_id: str, name: str = "", status: str = "",
                  description: str = "") -> str:
...
def get_matter_billing_configuration(matter_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_matter_billing_configuration(matter_id), indent=2)


@mcp.tool()
def update_matter_billing_configuration(matter_id: str, billing_type: str = "",
                                         rate: float = 0.0) -> str:
    ...
    if rate:
        fields["rate"] = rate
    return json.dumps(SmokeBallClient().update_matter_billing_configuration(matter_id, **fields), indent=2)
...
def list_leads(limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().list_leads(limit=limit, offset=offset), indent=2)
...
def list_matter_types(limit: int = 100, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().list_matter_types(limit=limit, offset=offset), indent=2)
...
def get_stage_in_set(stage_set_id: str, stage_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_stage_in_set(stage_set_id, stage_id), indent=2)
...
def get_role_on_matter(matter_id: str, role_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_role_on_matter(matter_id, role_id), indent=2)
...
def add_role_to_matter(matter_id: str, role_name: str, contact_id: str = "") -> str:
    ...
    return json.dumps(SmokeBallClient().add_role_to_matter(matter_id, **fields), indent=2)
...
def update_role_on_matter(matter_id: str, role_id: str, contact_id: str = "") -> str:
    ...
    if contact_id:
        fields["contactId"] = contact_id
    return json.dumps(SmokeBallClient().update_role_on_matter(matter_id, role_id, **fields), indent=2)


@mcp.tool()
def remove_role_from_matter(matter_id: str, role_id: str) -> str:
    """Remove a role from a matter."""
    return json.dumps(SmokeBallClient().remove_role_from_matter(matter_id, role_id), indent=2)
...
def get_relationships_on_matter(matter_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_relationships_on_matter(matter_id), indent=2)


@mcp.tool()
def get_relationship_on_role(matter_id: str, role_id: str) -> str:
    """Get the contact relationship for a specific role on a matter."""
    return json.dumps(SmokeBallClient().get_relationship_on_role(matter_id, role_id), indent=2)


@mcp.tool()
def add_relationship_to_role(matter_id: str, role_id: str,
                              contact_id: str, relation_type: str = "") -> str:
    """Add a contact relationship to a role on a matter."""
    fields = {"contactId": contact_id}
    if relation_type:
        fields["relationType"] = relation_type
    return json.dumps(SmokeBallClient().add_relationship_to_role(matter_id, role_id, **fields), indent=2)


@mcp.tool()
def update_relationship(matter_id: str, role_id: str, relationship_id: str,
                         relation_type: str) -> str:
    """Update a contact relationship on a matter role."""
    return json.dumps(SmokeBallClient().update_relationship(
        matter_id, role_id, relationship_id, relationType=relation_type), indent=2)


@mcp.tool()
def remove_relationship_from_role(matter_id: str, role_id: str,
                                   relationship_id: str) -> str:
    """Remove a contact relationship from a role on a matter."""
    return json.dumps(SmokeBallClient().remove_relationship_from_role(
        matter_id, role_id, relationship_id), indent=2)
...
def list_tasks(matter_id: str = "", limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_tasks(
        matter_id=matter_id or None, limit=limit, offset=offset), indent=2)
...
@mcp.tool()
def create_task(name: str, matter_id: str = "", due_date: str = "",
                assigned_to_id: str = "", notes: str = "") -> str:
...
def get_task_document(task_id: str, document_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_task_document(task_id, document_id), indent=2)
...
def create_task_document(task_id: str, file_id: str, name: str = "") -> str:
    ...
    if name:
        fields["name"] = name
    return json.dumps(SmokeBallClient().create_task_document(task_id, **fields), indent=2)


@mcp.tool()
def delete_task_document(task_id: str, document_id: str) -> str:
    """Remove a document attachment from a task."""
    return json.dumps(SmokeBallClient().delete_task_document(task_id, document_id), indent=2)
...
def list_events(matter_id: str = "", limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_events(
        matter_id=matter_id or None, limit=limit, offset=offset), indent=2)
...
@mcp.tool()
def create_event(name: str, start_date: str, end_date: str = "",
                 matter_id: str = "", location: str = "", notes: str = "") -> str:
...
@mcp.tool()
def update_event(event_id: str, name: str = "", start_date: str = "",
                 end_date: str = "", location: str = "") -> str:
...
@mcp.tool()
def create_event_reminder(event_id: str, minutes_before: int,
                           reminder_type: str = "Notification") -> str:
    """Create a reminder for an event. reminder_type: Notification | Email."""
    return json.dumps(SmokeBallClient().create_event_reminder(
        event_id, minutesBefore=minutes_before, reminderType=reminder_type), indent=2)


@mcp.tool()
def update_event_reminder(event_id: str, reminder_id: str, minutes_before: int) -> str:
    """Update an event reminder's lead time."""
    return json.dumps(SmokeBallClient().update_event_reminder(
        event_id, reminder_id, minutesBefore=minutes_before), indent=2)


@mcp.tool()
def delete_event_reminder(event_id: str, reminder_id: str) -> str:
    """Delete an event reminder."""
    return json.dumps(SmokeBallClient().delete_event_reminder(event_id, reminder_id), indent=2)
...
def list_memos_on_matter(matter_id: str, limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_memos_on_matter(
        matter_id, limit=limit, offset=offset), indent=2)
...
def list_fees(matter_id: str = "", limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_fees(
        matter_id=matter_id or None, limit=limit, offset=offset), indent=2)
...
def list_expenses(matter_id: str = "", limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_expenses(
        matter_id=matter_id or None, limit=limit, offset=offset), indent=2)
...
def list_invoices(matter_id: str = "", limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_invoices(
        matter_id=matter_id or None, limit=limit, offset=offset), indent=2)
...
def list_activity_codes(limit: int = 100, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_activity_codes(limit=limit, offset=offset), indent=2)
...
@mcp.tool()
def update_activity_code(code_id: str, code: str = "", description: str = "",
                          rate: float = 0.0) -> str:
    ...
    if rate:
        fields["rate"] = rate
    return json.dumps(SmokeBallClient().update_activity_code(code_id, **fields), indent=2)
...
def list_bank_accounts(limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_bank_accounts(limit=limit, offset=offset), indent=2)
...
def get_bank_account_matter_balances(account_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_bank_account_matter_balances(account_id), indent=2)


@mcp.tool()
def get_protected_bank_account_balance(account_id: str) -> str:
    """Get the protected (reserved) balance for a bank account."""
    return json.dumps(SmokeBallClient().get_protected_bank_account_balance(account_id), indent=2)


@mcp.tool()
def list_transactions(account_id: str, limit: int = 50, offset: int = 0) -> str:
    """List transactions for a bank account."""
    return json.dumps(SmokeBallClient().get_transactions(
        account_id, limit=limit, offset=offset), indent=2)


@mcp.tool()
def get_transaction(account_id: str, transaction_id: str) -> str:
    """Get a specific transaction from a bank account."""
    return json.dumps(SmokeBallClient().get_transaction(account_id, transaction_id), indent=2)


@mcp.tool()
def create_transaction(account_id: str, matter_id: str, amount: float,
                        description: str = "", transaction_type: str = "") -> str:
    ...
    return json.dumps(SmokeBallClient().create_transaction(account_id, **fields), indent=2)


@mcp.tool()
def create_requisition(account_id: str, matter_id: str, amount: float,
                        payee: str = "", description: str = "") -> str:
    ...
    if description:
        fields["description"] = description
    return json.dumps(SmokeBallClient().create_requisition(account_id, **fields), indent=2)


@mcp.tool()
def protect_funds(account_id: str, matter_id: str, amount: float,
                   reason: str = "") -> str:
...
def unprotect_funds(account_id: str, matter_id: str, amount: float) -> str:
    ...
    return json.dumps(SmokeBallClient().unprotect_funds(
        account_id, matterId=matter_id, amount=amount), indent=2)
...
def list_files_on_matter(matter_id: str, limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_files_on_matter(
        matter_id, limit=limit, offset=offset), indent=2)
...
def get_file_history(matter_id: str, limit: int = 50, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_file_history(
        matter_id, limit=limit, offset=offset), indent=2)
...
def add_file_to_matter(matter_id: str, name: str, folder_id: str = "") -> str:
    ...
    return json.dumps(SmokeBallClient().add_file_to_matter(matter_id, **fields), indent=2)
...
def get_file_preview_info_by_version(file_id: str, version_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_preview_info_by_version(file_id, version_id), indent=2)
...
def get_folder_contents(matter_id: str, folder_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_folder_contents(matter_id, folder_id), indent=2)


@mcp.tool()
def get_folder_path_hierarchy(matter_id: str, folder_id: str) -> str:
    """Get the full path hierarchy (breadcrumb) to a folder."""
    return json.dumps(SmokeBallClient().get_folder_path_hierarchy(matter_id, folder_id), indent=2)


@mcp.tool()
def get_folder_history(matter_id: str, limit: int = 50, offset: int = 0) -> str:
    """Get folder activity history for a matter."""
    return json.dumps(SmokeBallClient().get_folder_history(
        matter_id, limit=limit, offset=offset), indent=2)
...
def archive_matter(matter_id: str, reason: str = "") -> str:
    ...
    if reason:
        fields["reason"] = reason
    return json.dumps(SmokeBallClient().update_matter_archive(matter_id, **fields), indent=2)


@mcp.tool()
def unarchive_matter(matter_id: str) -> str:
    """Unarchive a matter."""
    return json.dumps(SmokeBallClient().update_matter_archive(matter_id, archived=False), indent=2)


@mcp.tool()
def patch_matter_archive(matter_id: str, retention_policy: str = "",
                          destruction_date: str = "") -> str:
    ...
    return json.dumps(SmokeBallClient().patch_matter_archive(matter_id, **fields), indent=2)
...
def list_referral_types(limit: int = 100, offset: int = 0) -> str:
    ...
    return json.dumps(SmokeBallClient().get_referral_types(limit=limit, offset=offset), indent=2)
...
@mcp.tool()
def update_authorization_group(group_id: str, name: str = "",
                                description: str = "") -> str:
    ...
    if description:
        fields["description"] = description
    return json.dumps(SmokeBallClient().update_authorization_group(group_id, **fields), indent=2)
...
def get_plugin_subscription(subscription_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_plugin_subscription(subscription_id), indent=2)
...
@mcp.tool()
def create_portal_task(title: str, matter_id: str, contact_id: str = "",
...
@mcp.tool()
def send_portal_message(matter_id: str, message: str,
                         contact_id: str = "") -> str:
...
def get_layout_on_matter(matter_id: str, layout_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_layout_on_matter(matter_id, layout_id), indent=2)
...
def add_layout_to_matter(matter_id: str, layout_design_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().add_layout_to_matter(
        matter_id, layoutDesignId=layout_design_id), indent=2)


@mcp.tool()
def add_contact_to_layout(matter_id: str, layout_id: str,
                           contact_id: str, role: str = "") -> str:
    """Add a contact to a layout instance (for merge field population)."""
    fields = {"contactId": contact_id}
    if role:
        fields["role"] = role
    return json.dumps(SmokeBallClient().add_contact_to_layout(matter_id, layout_id, **fields), indent=2)


@mcp.tool()
def get_layout_contacts(matter_id: str, layout_id: str) -> str:
    """Get contacts associated with a layout instance."""
    return json.dumps(SmokeBallClient().get_layout_contacts(matter_id, layout_id), indent=2)
...
def remove_layout_from_matter(matter_id: str, layout_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().remove_layout_from_matter(matter_id, layout_id), indent=2)
...
def get_matter_item(matter_id: str, item_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_item_on_matter(matter_id, item_id), indent=2)
...
def get_webhook_subscription(subscription_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().get_webhook_subscription(subscription_id), indent=2)


@mcp.tool()
def create_webhook_subscription(event_type: str, url: str,
                                  secret: str = "") -> str:
    """Create a webhook subscription. event_type: from list_webhook_event_types."""
    fields = {}
    if secret:
        fields["secret"] = secret
    return json.dumps(SmokeBallClient().create_webhook_subscription(
        event_type, url, **fields), indent=2)
...
def delete_webhook_subscription(subscription_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().delete_webhook_subscription(subscription_id), indent=2)
...
def test_webhook_subscription(subscription_id: str) -> str:
    ...
    return json.dumps(SmokeBallClient().notify_webhook_subscription(subscription_id), indent=2)
